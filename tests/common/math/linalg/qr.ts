import { describe, it, assert, assertCustomEqual, assertSameNumber } from '../../../_common.js'
...
describe('qr()', function() {
  ...
  it('decomposes a unit matrix', function() {
    ...
    const { q, r } = qr(matrix)
    assertCustomEqual(q.mult(r), matrix)
  })
  ...
  it('q is orthogonal', function() {
    ...
    const { q } = qr(matrix)
    assertCustomEqual(q.mult(q.transpose()), Matrix.unit(3))
  })
  ...
  it('r is upper triangular', function() {
    ...
    const { r } = qr(matrix)
    assertSameNumber(r.get(1, 0), 0)
    assertSameNumber(r.get(2, 0), 0)
    assertSameNumber(r.get(2, 1), 0)
  })
})
...
describe('det()', function() {
  ...
  it('computes the determinant of a 5x5 nonsingular matrix', function() {
    ...
    assertSameNumber(det(matrix), 2304)
  })
