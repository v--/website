...
describe('Matrix', function() {
  ...
  describe('#diagonal()', function() {
    it('constructs a diagonal matrix', function() {
      ...
      assertCustomEqual(matrix, expected)
    })
  })

  describe('#get()', function() {
    it('works correctly', function() {
      const matrix = new Matrix({ rows: 2, cols: 2, payload: [1, 0, 0, 1] })
      assert.strictEqual(matrix.get(0, 0), 1)
    })
  })
  ...
  describe('#add()', function() {
    it('adds two matrices elementwise', function() {
      const first = new Matrix({ rows: 2, cols: 2, payload: [1, 0, 0, 1] })
      const second = new Matrix({ rows: 2, cols: 2, payload: [0, 1, 1, 0] })
      const expected = new Matrix({ rows: 2, cols: 2, payload: [1, 1, 1, 1] })

      assertCustomEqual(first.add(second), expected)
    })
  })
  ...
  describe('#scale()', function() {
    it('scales a matrix', function() {
      ...
      assertCustomEqual(matrix.scale(3), expected)
    })
  })
  ...
  describe('#transpose()', function() {
    it('leaves a diagonal matrix', function() {
      const matrix = Matrix.diagonal([1, 2])
      assertCustomEqual(matrix.transpose(), matrix)
    })
    ...
    it('transposes a nonsquare matrix', function() {
      ...
      assertCustomEqual(matrix.transpose(), expected)
    })
  })
  ...
  describe('#mult()', function() {
    it('multiplies two square matrices', function() {
      ...
      assertCustomEqual(a.mult(b), expected)
    })
    ...
    it('is invariant under multiplication with unit matrices', function() {
      ...
      assertCustomEqual(unit.mult(matrix), matrix)
      assertCustomEqual(matrix.mult(unit), matrix)
    })
  })
})
