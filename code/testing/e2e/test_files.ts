      ...
      describe('table', function () {
        ...
        describe('the table header in the root page', function () {
          ...
          it('arranges the entry names by numeric file size when sorting by size', async function () {
            ...
            assert.deepEqual(sizesAndNames[3], { name: 'empty.txt',     size: '0 bytes' })
            assert.deepEqual(sizesAndNames[4], { name: 'sin.py',        size: '21 bytes' })
            assert.deepEqual(sizesAndNames[5], { name: 'dropsort.js',   size: '53 bytes' })
            assert.deepEqual(sizesAndNames[6], { name: 'lipsum.tex',    size: '125 bytes' })
            assert.deepEqual(sizesAndNames[7], { name: 'unlicense.txt', size: '1.18 KiB' })
            assert.deepEqual(sizesAndNames[8], { name: 'lipsum.txt',    size: '4.42 KiB' })
            assert.deepEqual(sizesAndNames[9], { name: 'cc0.txt',       size: '6.88 KiB' })
