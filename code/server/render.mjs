...
const render = overloader(
  {
    ...
    * impl (component) {
      ...
      for (const [key, value] of Object.entries(component.state.current)) {
        if (value === true) { yield ` ${key}` } else if (typeof value === 'string' && key !== 'text') { yield ` ${key}="${value}"` }
      }

      yield '>'

      if (component.isVoid) { return }

      if ('text' in component.state.current) { yield component.state.current.text }

      for (const child of component.children) {
        if (child instanceof Component) { yield * render(child) }
      }
