...
export function curveCanvas ({ width, height, mapping, curves, fitters, updateMapping, deleteMapping }) {
  ...
  return s(
    ...
    s(
      'g',
      { class: 'curves' },
      ...visibleCurves.map(function ({ curve, _fitter, cssClass }) {
        return s(
          'polyline',
          { class: 'curve ' + cssClass, points: join(' ', zip(domain, domain.map(x => curve.eval(x)))) }
        )
      })
    ),
