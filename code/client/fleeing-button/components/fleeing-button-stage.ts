import { fleeingButtonAttractors } from './fleeing-button-attractors.ts'
import { fleeingButtonMouseArea } from './fleeing-button-mouse-area.ts'
import { iconContent } from '../../../common/components/icon.ts'
import { Vec2D } from '../../../common/math/geom2d.ts'
import { Observable, combineLatest, map, pairwise, switchMap, withLatestFrom } from '../../../common/observable.ts'
...
export function fleeingButtonStage({ store }: IFleeingButtonStageState, env: ClientWebsiteEnvironment) {
  const _ = env.gettext.bindToBundle('fleeing-button')
