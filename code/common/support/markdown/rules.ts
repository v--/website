import { wildcard, term, neg, opt, rep, cat, alt, RefType } from '../../../common/support/parser.js'
...
function createBlockRule(start: string, end: string, matcher = alt(term('\\' + end), neg(end, '\n'))) {
  return cat(
    term(start),
    matcher,
    rep(matcher),
...
function createNestedBlockRule(start: string, end: string, rules: RefType[], matcher = alt(term('\\' + end), neg(end, '\n'))) {
