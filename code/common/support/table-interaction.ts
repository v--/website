...
export function parsePerPage(rawPerPage: string): uint32 {
  ...
  if (!Number.isInteger(perPage) || perPage < 1) {
    throw new PresentableError({
      errorKind: 'http',
      code: 400,
      details: { bundleId: 'interactive-table-error', key: 'error.details.per-page.invalid' },
    })
  }
...
export function parsePage(rawPage: string, pageCount: uint32): uint32 {
  ...
  if (!Number.isInteger(currentPage) || currentPage < 0) {
    throw new PresentableError({
      errorKind: 'http',
      code: 400,
      details: { bundleId: 'interactive-table-error', key: 'error.details.page.invalid' },
    })
  }
  ...
  if (currentPage >= pageCount) {
    throw new PresentableError({
      ...
      code: 400,
      details: {
        bundleId: 'interactive-table-error', key: 'error.details.page.out-of-bounds',
        context: { pageCount },
      },
    })
  }
...
export function parseSortStatus<T>(
  ...
  if (rawSortAsc !== undefined && rawSortDesc !== undefined) {
    throw new PresentableError({
      errorKind: 'http',
      code: 400,
      details: { bundleId: 'interactive-table-error', key: 'error.details.sort.conflict' },
    })
  }
  ...
  if (index === -1) {
    throw new PresentableError({
      ...
      code: 400,
      details: {
        bundleId: 'interactive-table-error', key: 'error.details.sort.unknown',
        context: {
