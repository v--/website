...
export function join (delimiter, iter) {
  try {
    ...
    if (e instanceof EmptyIterError) { return '' }
...
function * iterKeyValuePairs (object) {
  for (const [key, value] of Object.entries(object)) { yield `${key}: ${repr(value)}` }
}
...
export function repr (value) {
  ...
  if (value instanceof Function) { return value.name || 'anonymous' }

  if (value instanceof Array) { return '[' + join(', ', map(repr, value)) + ']' }
