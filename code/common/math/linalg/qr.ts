...
export function householderReflection(column: Matrix, k: uint32) {
  const n = column.rows
  ...
  for (let i = k; i < n; i++) {
    for (let j = k; j < n; j++) {
      const value = (i === j ? 1 : 0) - v.get(i, 0) * v.get(j, 0)
      h.set(i, j, value)
    }
  }
...
export function qr(matrix: Matrix) {
...
export function eigenvalues(matrix: Matrix, iterations: uint32 = 50) {
  if (matrix.rows !== matrix.cols) {
