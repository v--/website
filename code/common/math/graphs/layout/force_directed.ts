...
function clampToUnitCircle(vector: Vector) {
 ...
 */
export function getForceDirectedLayout<T>(graph: Graph<T>): GraphLayout<T> {
  const n = graph.order
  const area = 4 // The bounding square of the unit circle
  const k = Math.sqrt(area / n)
  const layout = new Map() as TCons.NonStrictMap<T, Vector>
  ...
  for (let temperature = 1; temperature > 0; temperature -= 1e-1) {
    const displacement = new Map() as TCons.NonStrictMap<T, Vector>
