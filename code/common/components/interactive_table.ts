...
export function sliceData({
  ...
  const comparator = function comparator(a: unknown, b: unknown) {
...
export function interactiveTable({
  ...
  const config = new QueryConfig<IQueryConfig>(path, Object.assign({ sorting: defaultSorting }, QUERY_CONFIG_DEFAULTS), QUERY_CONFIG_PARSERS)
  const perPage = config.get('per_page') as number
  const page = config.get('page') as number
  const sorting = config.get('sorting') as number
