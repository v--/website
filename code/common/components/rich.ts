...
import { Component, c, m } from '../rendering/component.ts'
...
class ComponentCreationVisitor extends RichTextVisitor<Component> {
  ...
  * iterChildren(entries?: IRichTextEntry[]) {
  ...
  override visitParagraphEntry(entry: IRichTextEntry & { kind: 'paragraph' }) {
    return c('p', this.visitTextState(entry), ...this.iterChildren(entry.children))
  }

  override visitContainerEntry(entry: IRichTextEntry & { kind: 'container' }) {
    return c('div', this.visitTextState(entry), ...this.iterChildren(entry.children))
  }

  override visitStrongEntry(entry: IRichTextEntry & { kind: 'strong' }) {
    return c('strong', this.visitTextState(entry), ...this.iterChildren(entry.children))
  }

  override visitEmphEntry(entry: IRichTextEntry & { kind: 'emph' }) {
    return c('em', this.visitTextState(entry), ...this.iterChildren(entry.children))
  }

  override visitHeadingEntry(entry: IRichTextEntry & { kind: 'heading' }) {
    return c(`h${entry.level}`, this.visitTextState(entry), ...this.iterChildren(entry.children))
  }
  ...
  * iterListItems(items: IRichTextEntry[], tight: boolean) {
  ...
  override visitListItemEntry(entry: IRichTextEntry & { kind: 'list_item' }, tight?: boolean) {
    if (tight) {
      ...
      return c('li', this.visitTextState(entry.children[0]), ...this.iterChildren(entry.children[0].children))
    }
  ...
  override visitImageEntry(entry: IRichTextEntry & { kind: 'image' }) {
    ...
    return c(icon, { refId: match.groups.refId, name: match.groups.name })
  }
  ...
  override visitDocument(doc: IRichTextDocument): Component {
    ...
    switch (this.config.mode) {
      case undefined:
      case 'normal':
        return c(this.config.rootTag ?? 'div', state, ...this.iterChildren(doc.entries))
      ...
      case 'paragraph':
        if (doc.entries.length === 1 && doc.entries[0].kind === 'paragraph') {
          ...
          if ('text' in entry) {
            return c(tag, { ...state, text: entry.text })
          }

          return c(tag, state, ...this.iterChildren(entry.children))
        }
